...
class TestParse(unittest.TestCase):
    ...
    def test_parse_fails_without_inputs(self):
        ...
        with self.assertRaises(SystemExit):
            args = malibuargs.parse(argv + ["-b1", "", "-b2", ""], None, None, None, None, None)

    def test_parse_fails_with_mixed_bam_fastq(self):
        argv = "submit -f tests/unittestconfig.txt".split()
        with self.assertRaises(SystemExit):
            args = malibuargs.parse(argv + ["-b1", "file1.bam"], None, None, None, None, None)

    def test_parse_fails_with_uneven_fastq(self):
        argv = "submit -f tests/unittestconfig.txt".split()
        with self.assertRaises(SystemExit):
            args = malibuargs.parse(argv + ["-b2", ""], None, None, None, None, None)

    def test_parse_fails_with_no_known_files(self):
        argv = "submit -f tests/unittestconfig.txt".split()
        with self.assertRaises(SystemExit):
            args = malibuargs.parse(argv + ["-b2", "file.txt", "-b1", "file.jpg"], None, None, None, None, None)
